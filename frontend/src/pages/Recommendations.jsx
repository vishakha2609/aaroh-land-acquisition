...
export default function Recommendations({ projects = [] }) {
  ...
  const generateRecommendations = (p) => {
    ...
    // Compensation
    if (p.compensationCompletion < 50) {
  ...
  return (
    <div>
      ...
      <h2>Actionable Decision Recommendations</h2>

      <p style={{
        color: 'var(--text-muted)',
        marginBottom: '20px'
      }}>
        Automated rule-based recommendations tailored for high-risk projects.
      </p>

      {highRiskProjects.length === 0 ? (

        <div className="card">
          <h3>No Immediate Recommendations</h3>
        ...
        highRiskProjects.map(p => (
          ...
          <div key={p.id} className="card">
            ...
            <div style={{
              ...
              <Lightbulb color="var(--saffron-accent)" />
            ...
            <div style={{
              ...
              <strong>Risk Level:</strong> {p.riskLevel}
              {' | '}
              <strong>Risk Score:</strong> {p.riskScore}/100
              {' | '}
              <strong>Delay Probability:</strong> {p.delayProbability}%
            </div>
