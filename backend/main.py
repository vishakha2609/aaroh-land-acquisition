...
app.add_middleware(
    ...
    # Allows any localhost port during local development
    allow_origin_regex=r"https?://localhost(:\d+)?",

    # Production frontend
    allow_origins=[
        "https://land-acquisition-sih-aaroh.netlify.app",
    ],
...
def predict_project(project: ProjectCreate):
    ...
    input_data = pd.DataFrame([{
        ...
        "legal_dispute":
            int(project.legal_dispute),

        "court_cases":
            project.court_cases,

        "ownership_conflict":
            int(project.ownership_conflict),
...
def update_project(
    ...
    existing_project.land_area = (
        project.land_area
    )

    existing_project.affected_families = (
        project.affected_families
    )

    existing_project.landowners = (
        project.landowners
    )
